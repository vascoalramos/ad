...
FROM netflix_dataset
	JOIN dim_rating on netflix_dataset.rating = dim_rating.rating;
    
-- populate fact_netflix_data
INSERT INTO fact_netflix_data (release_year, id_added_date, id_dim_type, id_dim_details, imdb_rating, imdb_rating_votes)
SELECT distinct release_year, dim_date.id, dim_type.id, dim_details.id, IMDB_rating, IMDB_rating_votes
FROM netflix_dataset
	JOIN dim_date on netflix_dataset.date_added = dim_date.`date`
    JOIN dim_type on netflix_dataset.`type` = dim_type.`type`
    JOIN dim_details on netflix_dataset.title = dim_details.title
		AND netflix_dataset.`description` = dim_details.`description`
        AND netflix_dataset.duration = dim_details.duration
	JOIN dim_rating on dim_details.id_rating = dim_rating.id;